...
class MatchController(QObject):
    ...
    def configure_match(self, payload: SetupPayload) -> MatchState:
        ...
        self._current_non_striker = payload.opening_non_striker
    ...
    def next_batter_options(self) -> list[str]:
        ...
        unavailable = dismissed | {name for name in [self._current_striker, self._current_non_striker] if name}
    ...
    def submit_ball(self, batting_input: int, bowling_input: int) -> None:
        try:
            ...
            if not self._current_striker or not self._current_non_striker or not self._current_bowler:
                raise ValueError("Current striker, non-striker, and bowler must be selected.")
            ...
            if innings_after != innings_before and result["snapshot"].get("status") == "live":
                ...
                if len(lineup) >= 2:
                    ...
                    self._current_non_striker = lineup[1]
                self._needs_next_batter = False
                self._needs_next_bowler = True
    ...
    def _decorate_snapshot(self, snapshot: dict) -> dict:
        ...
        snapshot["current_non_striker"] = self._current_non_striker or "-"
