...
class MatchSetupDialog(QDialog):
    ...
    def _build_ui(self) -> None:
        ...
        lineup = QFormLayout()
        self.home_order_edit = QTextEdit("A. Ray, K. Sen, M. Roy, P. Das, T. Khan")
        self.away_order_edit = QTextEdit("R. Das, J. Ali, N. Paul, S. Roy, B. Khan")
        ...
        self.non_striker_combo = QComboBox()
        self.opening_bowler_combo = QComboBox()
        opening.addRow("Opening striker", self.striker_combo)
        opening.addRow("Opening non-striker", self.non_striker_combo)
    ...
    def _on_accept(self) -> None:
        ...
        non_striker = self.non_striker_combo.currentText().strip()
        ...
        if home == away:
            ...
            return
        if len(home_order) < 2 or len(away_order) < 2:
            QMessageBox.warning(self, "Validation", "Each team needs at least two players in batting order.")
            return
        if not striker or not non_striker or striker == non_striker:
            QMessageBox.warning(self, "Validation", "Choose two different opening batters.")
            return
    ...
    def payload(self) -> MatchController.SetupPayload:
        return MatchController.SetupPayload(
            ...
            opening_non_striker=self.non_striker_combo.currentText().strip(),
            opening_bowler=self.opening_bowler_combo.currentText().strip(),
        )
